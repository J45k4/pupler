...
const readStringList = (data: FormData, name: string) =>
	data.getAll(name).map(String).map((value) => value.trim()).filter(Boolean);
...
const fillClockifyFilterSelects = (
		...
		return options.projects
			.filter(
				(project) =>
					...
					(project.client_id !== null && selectedClientIds.has(project.client_id)),
			)
			.map((project) => ({
				value: project.id,
	...
	const syncProjectOptions = () => {
		...
		projectSelect.setOptions(projectOptions()).setValues(projectSelect.values);
	};
	...
	if (clientRoot) {
		...
		const clientOptions: SearchSelectOption[] = options.clients.map((client) => ({
			value: client.id,
			label: client.name,
		}));
		clientSelect.setOptions(clientOptions).setValues(params.client_ids ?? []);
	...
	if (projectRoot) {
		...
		projectSelect.setOptions(projectOptions()).setValues(params.project_ids ?? []);
...
const loadImportSchedulesPage = async () => {
	...
	try {
		const [integrations, schedules, clients] = await Promise.all([
			...
			apiJson<Client[]>("/api/clients?archived_at=null&sort=name&order=asc"),
		]);
		fillIntegrationSelect(
			integrations.filter((integration) => integration.provider === PROVIDER_CLOCKIFY),
		);
...
const attachImportScheduleEvents = () => {
	...
	document.getElementById("import-schedules-list")?.addEventListener("click", async (event) => {
		...
		try {
			...
			setStatus(
				"import-schedules-status",
				error instanceof Error ? error.message : "Failed to queue import job.",
				true,
			);
		} finally {
...
const loadImportScheduleDetailPage = async (scheduleId: number) => {
	...
	try {
		...
		if (timezone instanceof HTMLInputElement) timezone.value = schedule.timezone;
		if (lookbackDays instanceof HTMLInputElement) {
			lookbackDays.value =
				params.lookback_days === null ? "" : String(params.lookback_days ?? 14);
		}
...
const attachImportScheduleDetailEvents = (scheduleId: number) => {
	...
	document.getElementById("import-schedule-run-button")?.addEventListener("click", async () => {
