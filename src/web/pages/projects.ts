...
const findClientByName = (clients: Client[], name: string) => {
	...
	return clients.find(
		(client) => normalizeName(client.name) === normalizedName,
	)
}
...
const renderProjectRows = () => {
	...
	for (const project of projects) {
		...
		clientSelect
			.setOptions(
				clientOptions(projectsPageState.clients, project.client_id),
			)
			.setValue(
				project.client_id === null ? null : String(project.client_id),
			)
		...
		clear.addEventListener("click", async () => {
			try {
				...
				setStatus(
					...
					error instanceof Error
						? error.message
						: "Failed to clear project client.",
					true,
				)
			}
		})
...
const attachProjectEvents = () => {
	...
	getElementById("project-create-client")?.replaceWith(
		createClientSelect.root,
	)
	document
...
export const renderProjectsPage = () => {
	...
	const archived = createElement("input", {
	...
	const form = createElement(
		...
		createElement(
			...
			{},
			"Client",
			createElement("span", { id: "project-create-client" }),
		),
		createElement(
			"div",
			{ className: "actions" },
	...
	page.append(
		createElement(
			"section",
			{ className: "workspace workspace--single projects-workspace" },
			createElement(
				"div",
