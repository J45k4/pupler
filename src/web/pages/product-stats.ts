import { formatMoney, renderPage, setStatus } from "../app"
...
const renderProductStatsRows = () => {
	...
	for (const row of rows) {
		...
		tableRow.append(
			...
			createElement("td", {
				text: formatUnitQuantities(row.used_quantities),
			}),
		)
			...
			createElement(
				...
				createElement(
					...
					createElement(
						"div",
						{},
						createElement("h2", {}, "Product Stats"),
					),
					productsLink,
				),
				createElement("div", {
					id: "product-stats-status",
					className: "status",
				}),
				createElement("div", {
					id: "product-stats-results",
					className: "results",
				}),
			),
		),
