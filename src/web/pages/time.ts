...
type TimeWeeklyViewMode = "summary" | "detail"

type TimeEntryRangeInput = {
	startedAt: string
	endedAt: string
}
...
const timeApi = {
	...
	fetchEntries: (filters: { from?: string; to?: string } = {}) => {
		...
		return timeApiJson<TimeEntry[]>(`/api/time-entries?${query.toString()}`)
	},
	...
	startEntry: (values: { project_id?: number; description: string | null }) =>
	...
	for (const entry of entries) {
		...
		if (existing) {
			...
			if (
				...
				Date.parse(existing.latest_started_at)
			) {
...
const findPreviousTimeEntryEnd = (
	...
	const previousEntry = entries
		.filter(
			(entry) => entry.id !== runningEntry.id && entry.ended_at !== null,
		)
		.sort(
			(first, second) =>
				Date.parse(second.ended_at ?? "") -
				Date.parse(first.ended_at ?? ""),
		)[0]
	return previousEntry?.ended_at ?? null
}

const projectOptions = (projects: Project[], selectedId?: number | null) =>
	projects
		.filter(
			(project) =>
				project.archived_at === null || project.id === selectedId,
		)
		.map((project) => ({
...
const selectProject = (projects: Project[], selectedId?: number | null) => {
...
const clientOptions = (
	...
	clients
		.filter(
			(client) => client.archived_at === null || client.id === selectedId,
		)
		.map((client) => ({
...
const findChoiceByName = (choices: ProjectChoice[], name: string) => {
	...
	return choices.find(
		(choice) =>
			normalizeProjectName(choice.project.name) === normalizedName,
	)
}
...
const selectedClientFromSelect = (
	...
	return clientSelect.text
		? (findClientByName(clients, clientSelect.text) ?? null)
		: null
}
...
class TimeActions {
	...
	public async ensureProject(projectName: string, clientName = "") {
		...
		const client = clientName.trim()
			? await this.ensureClient(clientName)
			: null
		const state = this.store.get()
		const existing = findChoiceByName(
			client
				? state.choices.filter(
						(choice) => choice.project.client_id === client.id,
					)
				: state.choices,
			normalizedName,
		)?.project
		if (existing) return existing
		const project = await timeApi.createProject(
			normalizedName,
			client?.id ?? null,
		)
	...
	public async stopTimer(entry: TimeEntry) {
		...
		this.store.setData(
			state.projects,
			replaceTimeEntry(state.entries, stopped),
		)
		return stopped
	}
	...
	public async updateRunningTimer(
		...
		this.store.setData(
			state.projects,
			replaceTimeEntry(state.entries, updated),
		)
		return updated
	}
...
const createTimeEntryCreateModal = (
	context: TimePageContext,
	options: { onEntryAdded?: () => void | Promise<void> } = {},
) => {
	...
	const close = () => {
		root.hidden = true
		root.classList.remove("time-entry-create-modal--range-preview")
	...
	const open = (range?: TimeEntryRangeInput) => {
		const nextRange = range ?? defaultTimeEntryRange()
		...
		startedAt.value = nextRange.startedAt
		endedAt.value = nextRange.endedAt
		root.classList.toggle(
			"time-entry-create-modal--range-preview",
			range !== undefined,
		)
		root.hidden = false
...
const createRunningEditModal = (
	...
	projectSelect
		.setOptions(
			choiceOptionsForClient(
				state.choices,
				selectedClientFromSelect(state.clients, clientSelect),
			),
		)
		.setValue(
			runningEntry.project_id === null
				? null
				: String(runningEntry.project_id),
		)
	...
	const startedAt = document.createElement("input")
	startedAt.type = "datetime-local"
	startedAt.value = formatTimestampForDateTimeLocalInput(
	...
	if (previousEndedAt) {
		...
		previousButton.onClick = async () => {
			try {
				startedAt.value =
					formatTimestampForDateTimeLocalInput(previousEndedAt)
				await context.actions.updateRunningStart(
					runningEntry,
					previousEndedAt,
				)
...
const createTimerPanel = (context: TimePageContext) => {
	...
	const render = () => {
		...
		if (runningEntry) {
			...
			const stopProjectModal = createStopTimerProjectModal(
				context,
				runningEntry,
			)
...
const createTimeWeeklySummary = (days: TimeWeeklyDayReport[]) => {
	...
	const total = div()
	total.append(
		text("span", "Total Time"),
		text("strong", formatDuration(totalSeconds)),
	)
	const average = div()
	average.append(
		...
		text(
			"strong",
			activeDays > 0
				? formatDuration(totalSeconds / activeDays)
				: "No active days",
		),
	)
	...
	clientCount.append(
		text("span", "Clients"),
		text("strong", String(clients.size)),
	)
	const span = div()
	span.append(
		text("span", "Week"),
...
const createTimeWeeklyChart = (
	...
	const { clients, dayClientSeconds, dayTotals } =
		getTimePeriodChartData(days)
	...
	for (const [index, day] of days.entries()) {
		...
		bar.style.setProperty(
			"--time-weekly-fill",
			maxDaySeconds > 0
				? `${Math.max(4, (dayTotal / maxDaySeconds) * 100)}%`
				: "0%",
		)
		if (dayTotal <= 0) {
		...
		label.append(
			text("strong", day.shortLabel),
			text("span", formatDuration(dayTotal)),
		)
...
const projectForWeeklyEntry = (entry: TimeEntry) =>
	entry.project ??
	(entry.project_id === null
		? null
		: {
...
const createTimeWeeklyDetailEntry = (
	...
	const entryEndMs = Date.parse(entry.ended_at ?? new Date().toISOString())
	const startMs = Math.max(dayStartMs, entryStartMs)
	const endMs = Math.min(dayEndMs, entryEndMs)
	if (endMs <= startMs || dayEndMs <= dayStartMs) return null
	...
	projectLine.append(
		timeColor(project?.color ?? UNKNOWN_TIME_COLOR),
		text(
			"span",
			`${project?.name ?? "No project"}${client ? ` - ${client.name}` : ""}`,
		),
	)
	const meta = div("time-weekly-detail-entry__meta")
	meta.append(
		text(
			"span",
			`${formatTimeOfDay(startDate)} - ${formatTimeOfDay(endDate)}`,
		),
		text("span", formatDuration(Math.floor((endMs - startMs) / 1000))),
	)
	root.append(titleElement, projectLine, meta)
	return root
}

const createTimeWeeklyDetail = (
	days: TimeWeeklyDayReport[],
	entries: TimeEntry[],
	...
	header.append(div("time-weekly-detail__corner"))
	for (const day of days) {
		const date = localDateFromInput(day.date) ?? new Date(day.from)
		const dayHeading = div("time-weekly-detail__day-heading")
		dayHeading.append(
			text("strong", `${formatWeekday(date)}, ${formatShortDate(date)}`),
			text("span", formatDuration(day.report?.total_seconds ?? 0)),
		)
		header.append(dayHeading)
	}

	const body = div("time-weekly-detail__body")
	const hours = div("time-weekly-detail__hours")
	for (let hour = 0; hour < 24; hour += 1) {
		const label = text("span", `${String(hour).padStart(2, "0")}:00`)
		hours.append(label)
	}
	body.append(hours)
	...
	const snapMinute = (minute: number) =>
		Math.max(0, Math.min(24 * 60, Math.round(minute / 15) * 15))

	const minuteFromPointer = (column: HTMLElement, event: PointerEvent) => {
		const rect = column.getBoundingClientRect()
		return snapMinute(((event.clientY - rect.top) / rect.height) * 24 * 60)
	}

	const selectionRange = (selection: NonNullable<typeof activeSelection>) => {
	...
	const rangeInputForSelection = (
		selection: NonNullable<typeof activeSelection>,
	) => {
		const { startMinute, endMinute } = selectionRange(selection)
		const dayStart = new Date(selection.day.from)
		return {
			startedAt: formatDateTimeLocalInput(
				new Date(dayStart.getTime() + startMinute * 60 * 1000),
			),
			endedAt: formatDateTimeLocalInput(
				new Date(dayStart.getTime() + endMinute * 60 * 1000),
			),
		}
	}
	...
	const updateSelectionElement = (
		...
		const { startMinute, durationMinutes } = selectionRange(selection)
		...
		selection.element.style.setProperty(
			"--selection-height",
			`${(Math.max(15, durationMinutes) / 1440) * 100}%`,
		)
		const label = selection.element.querySelector<HTMLElement>(
			".time-weekly-detail-selection__label",
		)
		if (label) {
			label.textContent = `${formatTimeOfDay(
				new Date(
					new Date(selection.day.from).getTime() +
						startMinute * 60 * 1000,
				),
	...
	const clearSelections = () => {
		root.querySelectorAll(".time-weekly-detail-selection").forEach(
	...
	for (const day of days) {
		const column = div("time-weekly-detail__day")
		column.addEventListener("pointerdown", (event) => {
			if (event.button !== 0) return
			if (
				event.target instanceof Element &&
				event.target.closest(
					".time-weekly-detail-entry, .time-weekly-detail-selection",
				)
			) {
				return
			}
			event.preventDefault()
			clearSelections()
			const selection = div("time-weekly-detail-selection")
			const label = div("time-weekly-detail-selection__label")
			...
			activeSelection = {
				...
				column,
				element: selection,
				startMinute: minuteFromPointer(column, event),
				endMinute: minuteFromPointer(column, event),
				dragging: true,
			}
			column.setPointerCapture(event.pointerId)
			updateSelectionElement(activeSelection)
		})
		column.addEventListener("pointermove", (event) => {
			if (
				!activeSelection ||
				activeSelection.column !== column ||
				!activeSelection.dragging
			) {
				return
			}
			activeSelection.endMinute = minuteFromPointer(column, event)
			updateSelectionElement(activeSelection)
		})
		column.addEventListener("pointerup", (event) => {
			if (!activeSelection || activeSelection.column !== column) return
			activeSelection.endMinute = minuteFromPointer(column, event)
			activeSelection.dragging = false
			updateSelectionElement(activeSelection)
			const { durationMinutes } = selectionRange(activeSelection)
			...
			const addButton = new Button({
				text: "Add",
				className: "primary time-weekly-detail-selection__add",
				type: "button",
			})
			const selectedRange = rangeInputForSelection(activeSelection)
			const openSelectedRange = () => {
				onAddRange(selectedRange)
			}
			addButton.root.addEventListener("pointerdown", (buttonEvent) => {
				buttonEvent.preventDefault()
				buttonEvent.stopPropagation()
				openSelectedRange()
			})
			addButton.root.addEventListener("click", (buttonEvent) => {
				buttonEvent.preventDefault()
				buttonEvent.stopPropagation()
			})
			activeSelection.element.append(addButton.root)
		...
		for (const entry of entries) {
			const entryElement = createTimeWeeklyDetailEntry(
				entry,
				day,
				onEditEntry,
			)
	...
	if (!entries.length) {
		const empty = div("empty time-weekly-detail__empty")
		empty.textContent = "No time entries in this week."
...
const createTimeMonthlyDayDetails = (
	...
	title.append(
		text(
			"span",
			`${formatDuration(day.report?.total_seconds ?? 0)} tracked`,
		),
	)
...
const createTimeMonthlyTotals = (
	...
	for (const client of clients) {
		const percentage =
			totalSeconds > 0
				? Math.round((client.totalSeconds / totalSeconds) * 100)
				: 0
		const row = div("time-monthly-detail__row")
		const label = div("time-entry-row__title")
		label.append(timeColor(client.color), text("strong", client.name))
...
const createTimeMonthlyCalendar = (
	...
	for (const [index, day] of days.entries()) {
		...
		header.append(
			...
			text(
				"span",
				formatWeekday(
					localDateFromInput(day.date) ?? new Date(day.from),
				),
			),
		)
		const total = text(
			"span",
			dayTotal > 0 ? formatDuration(trackedSeconds) : "Future",
		)
	...
	const monthTotalSeconds = dayTotals.reduce(
...
const renderTimeWeeklyReport = (
	...
	if (viewMode === "detail") {
		...
		resultsRoot.append(
			createTimeWeeklyChart(days, selectedDate, onSelectDate),
		)
	}
}
...
export const renderTimeOverviewPage = (page: HTMLElement) => {
	...
	const load = async (selection: TimeOverviewSelection) => {
		...
		try {
			...
			renderTimeOverviewReport(
				...
				averageReport && averagePeriod
					? {
							...
							baselineSeconds:
								countFullLocalDays(averagePeriod) * DAY_SECONDS,
			...
			setStatus(
				status,
				`Loaded ${selection.span.label.toLowerCase()} time usage.`,
			)
		} catch (error) {
			setStatus(
				status,
...
export const renderTimeWeeklyPage = (page: HTMLElement) => {
	...
	const controls = div(
		"spending-breakdown-controls time-overview-controls time-weekly-controls",
	)
	const previousWeek = new Button({
		text: "Previous Week",
		className: "secondary",
	})
	...
	weekInput.setAttribute("aria-label", "Week")
	const viewSelect = document.createElement("select")
	viewSelect.id = "time-weekly-view-select"
	viewSelect.value = selectedViewMode
	viewSelect.setAttribute("aria-label", "View")
	for (const option of [
		{ value: "summary", label: "Summary" },
		{ value: "detail", label: "Detail" },
	] satisfies { value: TimeWeeklyViewMode; label: string }[]) {
		const element = document.createElement("option")
		element.value = option.value
		element.textContent = option.label
		element.selected = option.value === selectedViewMode
		viewSelect.append(element)
	}
	controls.append(previousWeek.root, weekInput, viewSelect, nextWeek.root)

	const results = div("time-overview-results")
	const toolbar = div("time-weekly-toolbar")
	...
	const defaultSelectedDayDate = (days: TimeWeeklyDayReport[]) => {
		...
		return days.some((day) => day.date === today)
			? today
			: ((days.find((day) => day.baselineSeconds > 0) ?? days[0])?.date ??
					today)
	}
	...
	const load = async (weekDate: string) => {
		...
		try {
			...
			const reportsPromise = Promise.all(
				days.map((day) =>
					Date.parse(day.from) >= Date.now()
						? Promise.resolve(
								createEmptyTimeReport(day.from, day.to),
							)
						: timeApi.fetchReport(day.from, day.to),
				),
			)
			const entriesPromise = timeApi.fetchEntries({
				from: days[0]?.from,
				to: days[days.length - 1]?.to,
			})
			const clientsPromise = timeApi.fetchClients()
			const projectsPromise = timeApi.fetchProjects()
			const [reports, entries, clients, projects] = await Promise.all([
				reportsPromise,
				entriesPromise,
				clientsPromise,
				projectsPromise,
			...
			loadedDays = days.map((day, index) => ({
				...day,
				report:
					reports[index] ?? createEmptyTimeReport(day.from, day.to),
			}))
...
export const renderTimeMonthlyPage = (page: HTMLElement) => {
	...
	const previousMonth = new Button({
		text: "Previous Month",
		className: "secondary",
	})
