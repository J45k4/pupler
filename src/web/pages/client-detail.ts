...
const renderClientDetail = (client: Client, projects: Project[]) => {
	...
	const detailForm = createElement(
		...
		createElement(
			...
			"Color",
			createElement("input", {
				id: "client-detail-color",
				properties: { name: "color", type: "color", required: true },
			}),
		),
		createElement(
			"div",
	...
	const metadata = createElement(
		...
		createElement(
			...
			createElement("dd", { id: "client-detail-created" }),
		),
		createElement(
			"div",
			{},
			createElement("dt", {}, "Updated"),
			createElement("dd", { id: "client-detail-updated" }),
		),
	)
	page.replaceChildren(
		createElement(
			...
			createElement(
				"div",
				{},
				createElement("span", { className: "eyebrow" }, "Client"),
				createElement(
					"h1",
					{ className: "page-title" },
					createElement("span", {
						id: "client-detail-heading-color",
						className: "time-color",
					}),
					createElement("span", { id: "client-detail-heading-name" }),
				),
			),
			back,
		),
		createElement(
			...
			createElement(
				...
				createElement(
					...
					createElement("h2", {}, "Client Information"),
					createElement("span", { id: "client-detail-tag" }),
				),
				detailForm,
				createElement("div", {
			...
			createElement(
				...
				createElement(
					...
					createElement("span", {
						id: "client-project-count",
						className: "tag tag--neutral",
					}),
				),
				createElement("div", {
					id: "client-project-list",
					className: "client-project-list",
				}),
			),
		),
	)
	...
	if (statusTag) {
		statusTag.className =
			client.archived_at === null ? "tag" : "tag tag--neutral"
	...
	if (projectList) {
		if (!projects.length) {
			projectList.replaceChildren(
				createEmptyState("No projects for this client."),
			)
		} else {
			projectList.replaceChildren(
				...projects.map((project) => {
					const projectColor = createElement("span", {
						className: "time-color",
					})
					projectColor.style.setProperty(
						"--time-color",
						project.color,
					)
	...
	form?.addEventListener("submit", async (event) => {
		...
		try {
			const updatedClient = await apiJson<Client>(
...
export const renderClientDetailPage = async (
	...
	try {
		const [client, projects] = await Promise.all([
			...
			apiJson<Project[]>(
				`/api/projects?client_id=${clientId}&sort=name&order=asc`,
			),
		])
