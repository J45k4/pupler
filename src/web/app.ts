...
import { mountAppVersion } from "./update-widget"
...
export const renderAppShell = (shellClassName = "") => {
	const shellClasses = [
		...new Set(
			["page-shell", "page-shell--wide", shellClassName].filter(Boolean),
		),
	].join(" ")
	resetPage()
	...
	if (getCurrentUser()) mountAppVersion(navbarAbortController.signal)
	return main
}
...
export const attachExpirationPageEvents = () => {
	...
	results.addEventListener("click", async (event) => {
		...
		const itemName =
			consumeButton.dataset.consumeExpirationItemName ?? "item"
...
const createRecipeIngredientList = (ingredients: RecipeIngredient[]) => {
	if (!ingredients.length)
		return createEmptyState("No ingredients added yet.")
	return createElement(
		"div",
		{ className: "recipe-ingredient-list" },
		...ingredients.map((ingredient) => {
			const select = createElement("button", {
				className: "recipe-ingredient-item__select",
				properties: { type: "button" },
			})
			select.dataset.editRecipeIngredientId = String(ingredient.id)
			select.dataset.recipeIngredientName = encodeURIComponent(
				ingredient.name,
			)
			select.dataset.recipeIngredientQuantity = String(
				ingredient.quantity,
			)
			select.dataset.recipeIngredientUnit = encodeURIComponent(
				ingredient.unit,
			)
			select.dataset.recipeIngredientOptional = String(
				ingredient.is_optional,
			)
			select.dataset.recipeIngredientNotes = encodeURIComponent(
				ingredient.notes ?? "",
			)
			const defaultUnit =
			...
			const header = createElement(
				"div",
				{ className: "recipe-ingredient-item__header" },
			...
			const meta = createElement(
				"div",
				{ className: "recipe-ingredient-item__meta" },
				createElement(
					"span",
					{},
					`${ingredient.quantity} ${ingredient.unit}`,
				),
				defaultUnit && defaultUnit !== ingredient.unit
					? createElement("span", {}, `Default unit: ${defaultUnit}`)
					: null,
			)
			select.append(
				createElement(
					"div",
					{ className: "recipe-ingredient-item__main" },
					header,
					meta,
					ingredient.product
						? createElement(
								...
								`Product link: ${ingredient.product.name}`,
							)
						: null,
					ingredient.notes
						? createElement(
			...
			const remove = createElement(
				"button",
				{ className: "secondary", properties: { type: "button" } },
				"Remove",
			)
...
export const renderRecipeDetail = (recipe: Recipe) => {
		...
		? createElement(
				...
				...images.map((image) => {
					...
					return createElement(
						...
						createElement(
							"div",
							{ className: "recipe-image-card__meta" },
							createElement(
								"div",
								{},
								createElement(
									"strong",
									{},
									image.file.filename ?? `Image #${image.id}`,
								),
								createElement(
									"div",
									{ className: "section-copy" },
									formatReceiptDateTime(image.created_at),
								),
							),
							remove,
						),
	...
	const pictureForm = createElement(
		"form",
		{ id: "recipe-picture-form", className: "recipe-picture__form" },
		createUploadDropzone({
		...
		createElement(
			...
			createElement(
				"button",
				{ className: "secondary", properties: { type: "submit" } },
				"Upload Images",
			),
		),
	)
	...
	const detailForm = createElement(
		...
		createElement(
			"label",
			{ properties: { htmlFor: "recipe-detail-name" } },
			"Name",
			createElement("input", {
		...
		createElement(
			...
			createElement(
				"label",
				{ properties: { htmlFor: "recipe-detail-servings" } },
				"Servings",
				createElement("input", {
			...
			createElement(
				"label",
				{
					...
					properties: { htmlFor: "recipe-detail-is-active" },
				},
				active,
		...
		createElement(
			"label",
			{ properties: { htmlFor: "recipe-detail-description" } },
			"Description",
		...
		createElement(
			"label",
			{ properties: { htmlFor: "recipe-detail-instructions" } },
			"Instructions",
			createElement("textarea", {
	...
	const ingredientHeader = createElement(
		"div",
		{ className: "section-header recipe-detail-section__header" },
		createElement(
			"div",
			{ className: "recipe-ingredient-summary" },
			createElement("h2", {}, "Ingredients"),
			createElement(
				"span",
				{ className: "tag tag--neutral" },
	...
	const optional = createElement("input", {
		id: "recipe-ingredient-optional",
		properties: { name: "is_optional", type: "checkbox" },
	})
	const ingredientForm = createElement(
		"form",
		{
			id: "recipe-ingredient-modal-form",
			className: "recipe-ingredient-form",
		},
		createElement("input", {
			id: "recipe-ingredient-id",
			properties: { name: "ingredient_id", type: "hidden" },
		}),
		createElement(
			"label",
			{ properties: { htmlFor: "recipe-ingredient-name" } },
			"Ingredient",
			createElement("input", {
		...
		createElement(
			"div",
			{ className: "recipe-ingredient-form__row" },
			createElement(
				"label",
				{ properties: { htmlFor: "recipe-ingredient-quantity" } },
				"Quantity",
				createElement("input", {
			...
			createElement(
				"label",
				{
					className: "checkbox-toggle recipe-ingredient-form__toggle",
					properties: { htmlFor: "recipe-ingredient-optional" },
				},
				optional,
				createElement("span", {}, "Optional"),
			),
		),
		createElement(
			"label",
			{ properties: { htmlFor: "recipe-ingredient-notes" } },
			"Notes",
			createElement("input", {
	...
	const backdrop = createElement("div", {
		className: "recipe-ingredient-modal__backdrop",
		attributes: { "data-recipe-ingredient-modal-close": "" },
	})
	const modal = createElement(
		"div",
		{
			id: "recipe-ingredient-modal",
			className: "recipe-ingredient-modal",
		...
		createElement(
			"div",
			{
				className: "recipe-ingredient-modal__dialog card panel",
				attributes: {
					role: "dialog",
					"aria-modal": "true",
			...
			createElement(
				...
				createElement(
					"h2",
					{ id: "recipe-ingredient-modal-title" },
					"Add Ingredient",
				),
				close,
			),
			ingredientForm,
			createElement("div", {
				id: "recipe-ingredient-modal-status",
				className: "status",
			}),
		),
	)
	page.replaceChildren(
		...
		createElement(
			"section",
			{ className: "workspace recipe-detail-grid" },
			createElement(
				"div",
				{ className: "card panel" },
				createElement("h2", {}, "Images"),
				gallery,
				pictureForm,
				createElement("h2", {}, "Summary"),
				summary,
				createElement("div", {
					id: "recipe-picture-status",
					className: "status",
				}),
			),
			createElement(
				"div",
				{ className: "recipe-detail-stack" },
				createElement(
					"div",
					{ className: "card panel" },
					createElement(
						...
						createElement("h2", {}, "Recipe Details"),
						detailForm,
						createElement("div", {
							id: "recipe-detail-status",
							className: "status",
						}),
					),
				),
				createElement(
					...
					createElement(
						...
						{ className: "recipe-detail-section" },
						ingredientHeader,
						createRecipeIngredientList(ingredients),
						createElement("div", {
							id: "recipe-ingredient-status",
							className: "status",
						}),
					),
				),
...
const createProductCard = (product: Product) => {
	...
	if (product.is_perishable) {
		header.append(
			createElement("span", { className: "tag", text: "Perishable" }),
		)
	}
...
export const renderProductDetail = (product: Product) => {
	...
	const picture = createElement("img", {
		...
		properties: { src: pictureUrl, alt: product.name, loading: "lazy" },
	})
	const pictureRoot = createElement(
		"div",
		{ className: "receipt-picture" },
		picture,
	)
	picture.addEventListener("error", () =>
		pictureRoot.replaceChildren(
			createEmptyState("No product picture uploaded."),
		),
	)
	const pictureActions = createElement(
		"div",
		{ className: "actions" },
		createElement(
			"button",
	...
	if (product.picture_file)
		pictureActions.append(
			createElement(
				"button",
				{
					id: "product-picture-delete",
					className: "secondary",
					properties: { type: "button" },
				},
				"Remove Picture",
	...
	const pictureForm = createElement(
		"form",
		{ id: "product-picture-form", className: "product-picture__form" },
		createUploadDropzone({
	...
	const perishable = createElement(
		"select",
		{ id: "product-detail-is-perishable" },
		createElement(
			"option",
			{ properties: { value: "true", selected: product.is_perishable } },
			"true",
		),
		createElement(
			"option",
			{
				properties: {
					value: "false",
					selected: !product.is_perishable,
				},
			},
			"false",
		),
	...
	const detailForm = createElement(
		...
		createElement(
			...
			createElement("input", {
				...
				properties: { value: product.name, required: true },
			}),
		),
		createElement(
			"div",
		...
		createElement(
			...
			"Barcode",
			createElement("input", {
				id: "product-detail-barcode",
				properties: {
					value: product.barcode ?? "",
					placeholder: "6414893400012",
				},
			}),
		),
	...
	page.replaceChildren(
		createElement(
			...
			createElement(
				"div",
				{},
				createElement("span", { className: "eyebrow" }, "Product"),
			),
			back,
		),
		createElement(
			"section",
			{ className: "workspace product-detail-grid" },
			createElement(
				"div",
				{ className: "card panel" },
				createElement("h2", {}, "Picture"),
				pictureRoot,
				pictureForm,
				createElement("div", {
					id: "product-picture-status",
					className: "status",
				}),
			),
			createElement(
				...
				createElement(
					...
					createElement(
						"span",
						{
							className: product.is_perishable
								? "tag"
								: "tag tag--neutral",
						},
...
const getExpirationTag = (item: InventoryItem) => {
	...
	const daysUntilExpiration = Math.round((expiresDay - today) / 86_400_000)
	...
	root.replaceChildren(
		createElement(
			...
			sortedItems.map((item) =>
				renderExpirationItem(item, containersById),
			),
		),
	)
}
...
const getLast30DaysReceipts = (receipts: PurchaseReceipt[]) => {
	...
	return receipts.filter(
		(receipt) =>
			new Date(receipt.purchased_at).getTime() >= cutoff.getTime(),
	)
}
...
const getYearToDateReceipts = (receipts: PurchaseReceipt[]) => {
	...
	return receipts.filter(
		(receipt) =>
			new Date(receipt.purchased_at).getTime() >= yearStart.getTime(),
	)
}
...
const createSpendingTotalValues = (totals: Array<[string, number]>) =>
	createElement(
		"div",
		{ className: "dashboard-spending-total__values" },
		...(totals.length
			? totals.map(([currency, total]) =>
					createElement("strong", {}, formatMoney(total, currency)),
				)
...
const renderSpendingSummary = (
	...
	const total = (label: string, values: Array<[string, number]>) =>
		createElement(
			"div",
			{ className: "dashboard-spending-total" },
			createElement("span", {}, label),
			createSpendingTotalValues(values),
		)
	root.replaceChildren(
		createElement(
			"div",
			{ className: "dashboard-spending-summary" },
			total("Last 30 Days", last30DayTotals),
			total("Average Month Spending", averageMonthTotals),
			total("Average Weekly Spending", averageWeekTotals),
			total("Average Daily Spending", averageDayTotals),
			total("This Month Spending", currentMonthTotals),
			total("Year To Date Spending", yearToDateTotals),
			missingTotalCount
				? createElement(
						"div",
						{ className: "section-copy" },
						`${missingTotalCount} year-to-date receipt(s) have no total amount.`,
					)
				: null,
		),
	)
...
const renderDashboardTimer = (runningEntry: TimeEntry | null) => {
	...
	if (!runningEntry) {
		root.replaceChildren(
			createElement(
				"div",
				{ className: "dashboard-timer-empty" },
				createEmptyState("No timer running."),
				createElement(
					"a",
					{
						className: "primary action-link",
						properties: { href: "/time" },
						attributes: { "data-link": "" },
					},
					"Start Timer",
				),
	...
	color.style.setProperty(
		"--time-color",
		runningEntry.project?.color ?? "#2d7c6f",
	)
	...
	const stop = createElement(
		"button",
		{ className: "primary", properties: { type: "button" } },
		"Stop",
	)
	...
	root.replaceChildren(
		createElement(
			"div",
			{ className: "time-running dashboard-timer-running" },
			createElement(
				"div",
				{ className: "time-running__project" },
				color,
				createElement(
					"strong",
					{},
			...
			createElement(
				"div",
				{
					id: "dashboard-running-timer-duration",
					className: "time-running__duration",
				},
			...
			createElement(
				"div",
				{ className: "time-running__actions" },
				edit,
				stop,
			),
		),
...
const renderDashboardShoppingList = (items: ShoppingListItem[]) => {
	...
	root.replaceChildren(
		createElement(
			"div",
			{ className: "dashboard-shopping-list" },
			...activeItems.map((item) => {
				const href = createElement("a", {
					className: "dashboard-shopping-item",
					properties: { href: "/shoppinglist" },
					attributes: { "data-link": "" },
				})
				const pictureUrl = item.product?.picture_file_id
				...
				if (pictureUrl) {
					const image = createElement("img", {
						className: "dashboard-shopping-item__image",
						properties: {
							src: pictureUrl,
					...
					href.append(
						createElement("span", {
							className:
								"dashboard-shopping-item__image dashboard-shopping-item__image--placeholder",
						}),
					)
				href.append(
					createElement(
						"div",
						{ className: "dashboard-shopping-item__main" },
...
const renderInventoryTree = (
	...
	const renderInventoryItemNode = (item: InventoryItem) => {
		...
		return createElement(
			"li",
			{ className: "inventory-tree__leaf" },
	...
	const renderInventoryItemsList = (containerId: number | null) => {
		...
		return createElement(
			"ul",
			{ className: "inventory-tree__items" },
			...sortedItems.map(renderInventoryItemNode),
		)
	}
	...
	const createContainerNode = (
		...
		const toggle = hasChildren
			...
			: createElement("span", {
					className: "inventory-node__toggle-placeholder",
				})
		if (toggle instanceof HTMLButtonElement)
			toggle.dataset.toggleInventoryContainerId = String(container.id)
		const main = createElement(
			"div",
			{ className: "inventory-node__main" },
			createElement(
				"div",
				{ className: "inventory-node__title-row" },
				toggle,
				createElement("strong", {}, container.name),
			),
			createElement(
				"div",
				{ className: "inventory-node__meta" },
				...
				container.notes
					? createElement("span", {}, container.notes)
					: null,
			),
		)
		const open = createElement(
			"a",
			{
				className: "secondary action-link inventory-node__button",
				properties: { href: `/inventory/containers/${container.id}` },
				attributes: { "data-link": "" },
			},
			"Open",
		)
		...
		const node = createElement(
			...
			{
				className:
					"inventory-node inventory-node--container inventory-drop-target",
				properties: { draggable: true },
			},
			main,
			createElement(
				"div",
				{ className: "inventory-node__actions" },
				open,
				remove,
			),
		)
		Object.assign(node.dataset, {
			dragKind: "container",
			dragId: String(container.id),
			dropKind: "container",
			dropId: String(container.id),
		})
		const branch = createElement(
			"li",
			{ className: "inventory-tree__branch" },
			node,
		)
		if (!isCollapsed)
			branch.append(
				createElement(
					"div",
					{ className: "inventory-tree__children" },
					renderInventoryItemsList(container.id),
					childContainers.length
						? createElement(
								"ul",
								{ className: "inventory-tree__containers" },
								...childContainers.map(createContainerNode),
							)
						: null,
				),
			)
	...
	const mode = createElement(
		"select",
		{
			...
			className: "inventory-tree__filter",
			attributes: { "aria-label": "Inventory view" },
		},
		...(
			[
				["active", "Active"],
			...
			createElement(
				"option",
				{
					properties: {
						value,
						selected: inventoryItemMode === value,
					},
				},
				label,
			),
	...
	const add = createElement(
		"button",
		{
			className: "primary inventory-node__button",
			properties: { type: "button" },
			attributes: { "data-open-inventory-container-modal": "" },
		},
		"Add Container",
	)
	const content = createElement(
		"div",
		{ className: "inventory-tree__root-content" },
		!items.length
			? createElement(
					"div",
					{ className: "inventory-tree__empty" },
					getInventoryEmptyMessage(inventoryItemMode),
				)
			: null,
		unplacedItems.length ? renderInventoryItemsList(null) : null,
		topLevelContainers.length
			? createElement(
					"ul",
					{
						className:
							"inventory-tree__containers inventory-tree__containers--root",
					},
					...topLevelContainers.map(createContainerNode),
				)
			: null,
	...
	const inventoryRoot = createElement(
		"div",
		{ className: "inventory-root inventory-drop-target" },
		createElement(
			"div",
			{ className: "inventory-tree__toolbar" },
			createElement("div", {
				id: "inventory-status",
				className: "status",
			}),
...
const renderReceiptBoard = (
	...
	const scrollbar = createElement(
		"div",
		{
			className: "receipt-kanban-scrollbar",
			attributes: { "data-receipt-kanban-scrollbar": "" },
		},
		createElement("div", {
			className: "receipt-kanban-scrollbar__inner",
			attributes: { "data-receipt-kanban-scrollbar-inner": "" },
		}),
	)
	const board = createElement(
		"div",
		{
			className: "receipt-kanban",
			attributes: { "data-receipt-kanban": "" },
		},
		...columns.map((column) => {
			...
			const title =
				column.id === null
					...
					: createElement(
							"a",
							{
								className: "receipt-kanban__title-link",
								properties: { href: `/groups/${column.id}` },
								attributes: { "data-link": "" },
							},
							column.name,
						)
			const section = createElement(
				"section",
				{ className: "receipt-kanban__column receipt-drop-target" },
				createElement(
					"header",
					{ className: "receipt-kanban__header" },
				...
				createElement(
					"div",
					{ className: "receipt-kanban__list" },
					...(columnReceipts.length
						...
						: [
								createElement(
									"div",
									{ className: "receipt-kanban__empty" },
									"No receipts",
								),
							]),
				),
...
const renderReceiptGroupControls = (
	groups: Group[],
...
export const renderReceiptDetail = (
	...
	const pictureUrl = updated
		? `/api/receipts/${receipt.id}/picture?updated=${encodeURIComponent(updated)}`
		: `/api/receipts/${receipt.id}/picture`
	const back = createElement(
		"a",
		{
			className: "secondary action-link",
			properties: { href: "/receipts" },
			attributes: { "data-link": "" },
		},
		"Back To Receipts",
	)
	...
	const pictureRoot = createElement(
		...
		createElement(
			"button",
			{
				className: "receipt-picture__trigger",
				properties: { type: "button" },
				attributes: {
	...
	picture.addEventListener("error", () =>
		pictureRoot.replaceChildren(
			createEmptyState("No receipt picture uploaded."),
		),
	)
	const metadataValue = (label: string, value: Node | string) =>
		createElement(
			"div",
			{},
			createElement("dt", {}, label),
	...
	const metadata = createElement(
		"dl",
		{ className: "receipt-metadata" },
		metadataValue("Store", receipt.store_name),
		metadataValue(
			"Group",
			receipt.group
				? createElement(
		...
		metadataValue("Purchased", formatReceiptDateTime(receipt.purchased_at)),
		metadataValue("Currency", receipt.currency),
		metadataValue(
			"Total",
			formatMoney(receipt.total_amount, receipt.currency),
		),
		metadataValue("Created", formatReceiptDateTime(receipt.created_at)),
		metadataValue("Updated", formatReceiptDateTime(receipt.updated_at)),
	)
	const groupName = createElement("input", {
		id: "receipt-detail-group-name",
		properties: {
			value: receipt.group?.name ?? "",
			placeholder: "grocery",
		},
		attributes: { list: "receipt-detail-group-options" },
	})
	const groupOptions = createElement(
		"datalist",
		{ id: "receipt-detail-group-options" },
		...groups.map((group) =>
			createElement("option", { properties: { value: group.name } }),
		),
	)
	const groupForm = createElement(
		"form",
		{ id: "receipt-detail-group-form", className: "receipt-group-form" },
		createElement("label", {}, "Group", groupName, groupOptions),
		createElement(
			"div",
			{ className: "actions" },
			createElement(
	...
	const extractedItems = items.length
		? createElement(
				"table",
				{ className: "shoppinglist-table" },
				createElement(
					"thead",
					{},
					createElement(
						"tr",
						{},
						...["Product Name", "Quantity", "Line Total"].map(
							(label) => createElement("th", {}, label),
						),
					),
				),
				createElement(
		...
		: createEmptyState("No extracted line items yet.")
	...
	const modal = createElement(
		"div",
		{
			id: "receipt-picture-modal",
			className: "receipt-modal",
			properties: { hidden: true },
		},
		createElement("div", {
			className: "receipt-modal__backdrop",
			attributes: { "data-receipt-modal-close": "" },
		}),
		createElement(
			"div",
			{
			...
			createElement(
				"div",
				{ className: "receipt-modal__viewport" },
				modalImage,
			),
		),
	)
	page.replaceChildren(
		createElement(
			...
			createElement(
				"div",
				{},
				createElement("span", { className: "eyebrow" }, "Receipt"),
			),
			back,
		),
		createElement(
			"section",
			{ className: "workspace receipt-detail-grid" },
			createElement(
				"div",
				{ className: "card panel" },
				createElement("h2", {}, "Original Picture"),
				pictureRoot,
			),
			createElement(
				"div",
				{ className: "card panel" },
				createElement("h2", {}, "Extracted Metadata"),
				metadata,
				createElement("h2", {}, "Group"),
				groupForm,
				createElement("div", {
					id: "receipt-detail-group-status",
					className: "status",
				}),
				createElement("h2", {}, "Extracted Items"),
				extractedItems,
			),
		),
		modal,
...
export const attachReceiptDetailEvents = (
	...
	const modalImage = querySelector<HTMLImageElement>(".receipt-modal__image")
...
const fetchActiveShoppingListItems = async () => {
	...
	const response = await fetch(
		`/api/shopping-list-items?${params.toString()}`,
	)
	const body = (await response.json()) as
		| ShoppingListItem[]
		| { error?: string }
...
export const fetchReceipt = async (receiptId: number) => {
	...
	const body = (await response.json()) as PurchaseReceipt | { error?: string }
...
export const fetchAllReceiptItems = async () => {
	const response = await fetch(
		"/api/receipt-items?sort=created_at&order=desc",
	)
	const body = (await response.json()) as
		| PurchaseReceiptItem[]
		| { error?: string }
...
export const loadProducts = async () => {
	...
	try {
		...
		const response = await fetch(`/api/products${buildQuery(initialField)}`)
		const body = (await response.json()) as Product[] | { error?: string }
...
export const attachProductPageEvents = () => {
	...
	const modalCloseButtons = querySelectorAll("[data-product-modal-close]")
	...
	getElementById("product-picture-delete")?.addEventListener(
		...
		async () => {
			try {
				...
				setStatus(
					"product-picture-status",
					`Removed picture for ${updated.name}.`,
				)
			} catch (error) {
				setStatus(
					"product-picture-status",
...
const attachRecipeDetailEvents = (recipeId: number) => {
	...
	const ingredientQuantityInput = getElementById("recipe-ingredient-quantity")
	...
	const ingredientOptionalInput = getElementById("recipe-ingredient-optional")
	const ingredientModalTitle = getElementById("recipe-ingredient-modal-title")
	...
	const openIngredientEditModal = (editButton: HTMLElement) => {
		...
		ingredientIdInput.value =
			editButton.dataset.editRecipeIngredientId ?? ""
	...
	document
		...
		?.addEventListener("submit", async (event) => {
			...
			const quantityInput = getElementById("recipe-ingredient-quantity")
			const unitInput = getElementById("recipe-ingredient-unit")
			const notesInput = getElementById("recipe-ingredient-notes")
			const optionalInput = getElementById("recipe-ingredient-optional")
			...
			try {
				...
				const unit =
					unitInput.value.trim() || ingredient.default_unit || "pcs"
				if (ingredientId) {
	...
	document
		...
		?.addEventListener("submit", async (event) => {
			...
			const descriptionInput = getElementById("recipe-detail-description")
...
export const attachReceiptsPageEvents = () => {
	...
	const openGroupModalButton = getElementById("open-group-modal-button")
	...
	form?.addEventListener("submit", async (event) => {
		...
		const storeNameInput = getElementById("receipt-store-name")
		const purchasedAtInput = getElementById("receipt-purchased-at")
		const currencyInput = getElementById("receipt-currency")
		const totalAmountInput = getElementById("receipt-total-amount")
...
export const attachInventoryPageEvents = () => {
	...
	treeRoot.addEventListener("click", async (event) => {
		...
		if (consumeButton) {
			const itemId = Number(consumeButton.dataset.consumeInventoryItemId)
			if (!Number.isInteger(itemId)) {
				return
			}
...
export const renderInventoryItemDetail = (
	...
	const properties = createElement(
		"dl",
		{ className: "receipt-metadata" },
		row("Name", item.name),
		row("Quantity", String(item.quantity)),
		row("Unit", item.unit),
		row("Purchased", date(item.purchased_at)),
		row("Expires", date(item.expires_at)),
		row("Consumed", date(item.consumed_at)),
		row("Notes", item.notes ?? "-"),
		row("Created", formatReceiptDateTime(item.created_at)),
		row("Updated", formatReceiptDateTime(item.updated_at)),
	)
	...
	const productOption = (value: string, label: string) =>
		createElement(
			"option",
			{ properties: { value, selected: value === productSelected } },
			label,
		)
	if (!products.length && item.product_id === null)
		productSelect.append(productOption("", "No products in Pupler yet"))
	else {
	...
	const receiptSelect = createElement("select", {
		id: "inventory-item-receipt-item-id",
		properties: { name: "receipt_item_id" },
	})
	const receiptSelected =
		item.receipt_item_id === null ? "" : String(item.receipt_item_id)
	const receiptOption = (value: string, label: string) =>
		createElement(
			"option",
			{ properties: { value, selected: value === receiptSelected } },
			label,
		)
	if (!receiptItems.length && item.receipt_item_id === null)
		receiptSelect.append(receiptOption("", "No receipt rows in Pupler yet"))
	else {
		...
		if (
			...
			!receiptItems.some(
				(receiptItem) => receiptItem.id === item.receipt_item_id,
			)
		)
			receiptSelect.append(
				receiptOption(
	...
	const linksForm = createElement(
		"form",
		{ id: "inventory-item-links-form", className: "inventory-link-form" },
		createElement("label", {}, "Product", productSelect),
		createElement("label", {}, "Receipt Row", receiptSelect),
		createElement(
			"div",
			{ className: "actions" },
			createElement(
	...
	const locationValue =
		item.container_id === null
			? "Top level"
			: container
				? link(`/inventory/containers/${container.id}`, container.name)
				: `Container #${item.container_id}`
	const productValue =
		item.product_id === null
			? "-"
			: item.product
				? link(`/products/${item.product.id}`, item.product.name)
				: `Product #${item.product_id}`
	const ids = createElement(
		"dl",
		{ className: "receipt-metadata" },
		row("Inventory Item ID", String(item.id)),
		row("Location", locationValue),
		row(
			"Container ID",
			item.container_id === null ? "-" : String(item.container_id),
		),
		row("Product", productValue),
		row(
			"Product ID",
			item.product_id === null ? "-" : String(item.product_id),
		),
		...
		row(
			"Ingredient ID",
			item.ingredient_id === null ? "-" : String(item.ingredient_id),
		),
		row(
			"Receipt Row ID",
			item.receipt_item_id === null ? "-" : String(item.receipt_item_id),
	...
	const gallery = images.length
		? createElement(
				...
				{
					className:
						"recipe-image-gallery inventory-item-image-gallery",
				},
				...images.map((image) => {
					const remove = createElement(
						"button",
	...
	const pictureForm = createElement(
		"form",
		{
			id: "inventory-item-picture-form",
			className: "recipe-picture__form inventory-item-picture__form",
		},
		createUploadDropzone({
	...
	page.replaceChildren(
		...
		createElement(
			"section",
			{ className: "workspace inventory-item-detail-grid" },
			createElement(
				"div",
				{ className: "card panel" },
				createElement(
			...
			createElement(
				...
				createElement(
					"div",
					{ className: "section-header" },
					createElement("h2", {}, "Links & IDs"),
					createElement(
						"span",
						{ className: expiration.className },
						expiration.label,
					),
				),
				linksForm,
				createElement("div", {
					id: "inventory-item-links-status",
					className: "status",
				}),
				ids,
			),
			createElement(
				"div",
				{ className: "card panel inventory-item-images-panel" },
				createElement("h2", {}, "Images"),
				gallery,
				pictureForm,
				createElement("div", {
					id: "inventory-item-picture-status",
					className: "status",
				}),
			),
		),
...
export const attachInventoryItemDetailEvents = (
	...
	if (pictureForm instanceof HTMLFormElement) {
		pictureForm.addEventListener("submit", async (event) => {
			...
			const pictureInput = getElementById("inventory-item-picture-input")
			if (!(pictureInput instanceof HTMLInputElement)) {
				return
			}
