...
export class SearchSelect extends UiComponent<HTMLDivElement> {
	...
	constructor(options: {
		...
		this.root.dataset.searchSelectMultiple = options.multiple ? "true" : "false";
		if (options.name) {
...
const searchSelectItems = (root: HTMLElement): SearchSelectItem[] => {
	...
	const matches = state.options
		.filter(
			(option) =>
				!searchSelectIsMultiple(root) || !selectedValues.has(option.value),
		)
		.filter((option) =>
			normalizedQuery
...
const syncSearchSelectSelection = (root: HTMLElement) => {
	...
	if (!isMultiple) {
		...
		const selectedOption = selectedValue ? optionForValue(root, selectedValue) : null;
...
export const attachSearchSelects = (root: Document | HTMLElement) => {
	...
	root.addEventListener("mousedown", (event) => {
		...
		if (!searchRoot) return;
		if (shouldSkipSearchRoot(searchRoot)) return;
		const removeButton = target.closest<HTMLElement>("[data-search-select-remove]");
		if (removeButton) {
			event.preventDefault();
