...
const passwordHashFromBody = async (body: JsonObject) => {
	...
	const password = readOptionalBodyField(
		body,
		"password",
		expectNullableString,
	)
	if (passwordHash !== undefined && password !== undefined) {
		throw new HttpError(
			400,
			"Provide either `password` or `password_hash`, not both",
		)
	}
...
const parseCreateValues = async (body: JsonObject) => {
	...
	return {
		...
		username: normalizeNullableString(
			readOptionalBodyField(body, "username", expectNullableString) ??
				null,
		),
...
const parseReplaceValues = async (
	...
	return {
		...
		password_hash: await passwordHashFromBody(body),
		is_admin:
			readOptionalBodyField(body, "is_admin", expectBoolean) ??
			existingRow?.is_admin ??
			false,
...
const parsePatchValues = async (body: JsonObject) => {
	...
	const username = readOptionalBodyField(
		body,
		"username",
		expectNullableString,
	)
	...
	if (username !== undefined)
		values.username = normalizeNullableString(username)
	if (email !== undefined) values.email = normalizeNullableString(email)
	if (passwordHash !== undefined || password !== undefined)
		values.password_hash = await passwordHashFromBody(body)
