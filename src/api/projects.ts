...
const parseCreateValues = (body: JsonObject) => {
	...
	return {
		client_id:
			readOptionalBodyField(body, "client_id", expectNullableInteger) ??
			null,
		name,
		color,
		archived_at:
...
const parsePatchValues = (body: JsonObject) => {
	...
	const clientId = readOptionalBodyField(
		body,
		"client_id",
		expectNullableInteger,
	)
...
const parseMergeValues = (body: JsonObject) => {
	...
	const archiveSource =
		readOptionalBodyField(body, "archive_source", expectBoolean) ?? true
	const deleteSource =
		readOptionalBodyField(body, "delete_source", expectBoolean) ?? false
	if (archiveSource && deleteSource) {
		throw new HttpError(
			400,
			"`archive_source` and `delete_source` cannot both be true",
		)
	}
...
export const projectMergeRoute = async (req: BunRequest<string>) => {
	...
	const { sourceId, archiveSource, deleteSource } = parseMergeValues(
