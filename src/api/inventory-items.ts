...
export const inventoryItemsCollectionRoute = async (req: Request) => {
	if (req.method === "GET") {
	...
	if (req.method === "POST") {
		...
		return json(201, await fetchInventoryItemDetail(db, created.id))
	}
	throw new HttpError(405, "Method not allowed for this route")
}

export const inventoryItemDetailRoute = async (req: BunRequest<string>) => {
...
export const inventoryItemImagesCollectionRoute = async (
	...
	if (req.method === "POST") {
		...
		const uploaded = parseUploadedInventoryItemImages(
...
export const inventoryItemImageDetailRoute = async (
	req: BunRequest<string>,
) => {
	const inventoryItemId = parseIdParam(req.params.id)
	const pictureId = parseIdParam(req.params.pictureId)
	await ensureInventoryItemExists(db, inventoryItemId)
	const image = await fetchInventoryItemImage(db, inventoryItemId, pictureId)
	if (!image) {
