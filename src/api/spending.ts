...
const parseSpendingQuery = (url: URL): SpendingPeriod => {
	...
	if (rangeParam === "all") {
		if (fromParam !== null || daysParam !== null) {
			throw new HttpError(400, "Use `range=all` without `from` or `days`")
		}
		return {
			from: null,
			to,
	...
	const days = daysParam ? parseIntegerQuery("days", daysParam) : DEFAULT_DAYS
...
const calendarMonthStartDayIndex = (timestamp: string) => {
	...
	return Math.floor(Date.UTC(date.getFullYear(), date.getMonth(), 1) / DAY_MS)
}

const monthlyAverageTotals = (
	spendingByCurrency: Map<string, { firstDay: number; total: number }>,
