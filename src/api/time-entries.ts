...
const DEFAULT_SORT = [{ started_at: "desc" as const }, { id: "desc" as const }]
const SORT_FIELDS = [
	"id",
	"user_id",
	"project_id",
...
const parseFilters = (url: URL) => {
	...
	for (const [key, value] of url.searchParams.entries()) {
		...
		switch (key) {
			...
			case "description":
				where.description =
					value === "null" ? null : { contains: value }
				break
			case "started_at":
				where.started_at =
					value === "null" ? null : parseTimestampQuery(key, value)
				break
			case "ended_at":
				where.ended_at =
					value === "null" ? null : parseTimestampQuery(key, value)
				break
			case "running":
				where.ended_at = parseBooleanQuery(key, value)
					? null
					: { not: null }
				break
			case "from": {
...
const parseCreateValues = (body: JsonObject) => {
	...
	const endedAt =
		readOptionalBodyField(body, "ended_at", expectNullableTimestamp) ?? null
	const userId =
		readOptionalBodyField(body, "user_id", expectNullableInteger) ?? null
	const projectId =
		readOptionalBodyField(body, "project_id", expectNullableInteger) ?? null
...
const parsePatchValues = (
	...
	const projectId = readOptionalBodyField(
		body,
		"project_id",
		expectNullableInteger,
	)
	...
	const endedAt = readOptionalBodyField(
		body,
		"ended_at",
		expectNullableTimestamp,
	)
	...
	if (description !== undefined)
		values.description = normalizeDescription(description)
	...
	assertChronologicalRange(
		(typeof values.started_at === "string"
			? values.started_at
...
const parseStartValues = (body: JsonObject) => {
	...
	const startedAt =
		readOptionalBodyField(body, "started_at", expectTimestamp) ?? utcNow()
	return {
		user_id:
			readOptionalBodyField(body, "user_id", expectNullableInteger) ??
			null,
		project_id:
			readOptionalBodyField(body, "project_id", expectNullableInteger) ??
			null,
		description: normalizeDescription(
			readOptionalBodyField(body, "description", expectNullableString) ??
				null,
...
const createEntry = async (
	db: Database,
	values: ReturnType<typeof parseCreateValues>,
) => {
