...
const parseLookbackDays = (body: JsonObject) => {
	...
	if (
		...
		throw new HttpError(
			400,
			"Field `lookback_days` must be at least 1 or null",
		)
	}
...
const parseScheduleBody = (body: JsonObject) => {
	...
	const integrationId = requireBodyField(
		body,
		"integration_id",
		expectInteger,
	)
	...
	if (!isImportScheduleCadence(cadence)) {
		throw new HttpError(
			400,
			"Field `cadence` must be a valid import schedule cadence",
		)
	}
	...
	const nextRunAt =
		readOptionalBodyField(body, "next_run_at", expectNullableTimestamp) ??
		null
	...
	return {
		...
		params: {
			...
			dry_run:
				readOptionalBodyField(body, "dry_run", expectBoolean) ?? false,
			target_client_id:
				readOptionalBodyField(
					body,
					"target_client_id",
			...
			client_ids:
				...
				[],
			project_ids:
				readOptionalBodyField(body, "project_ids", expectStringArray) ??
				[],
		},
	}
}
...
export const importSchedulesCollectionRoute = (db: Database) =>
	withErrorHandling(async (req: Request) => {
		...
		if (req.method === "POST") {
			...
			if (!integration)
				throw new HttpError(400, "Integration does not exist")
...
export const importScheduleDetailRoute = (db: Database) =>
	withErrorHandling(async (req: BunRequest<string>) => {
		...
		if (req.method === "PATCH") {
			...
			const integrationId = readOptionalBodyField(
				body,
				"integration_id",
				expectInteger,
			)
			if (integrationId !== undefined) {
				const integration =
					await db.client.externalIntegration.findUnique({
				...
				if (
					...
					ExternalIntegrationProvider.Clockify
				) {
					throw new HttpError(
						400,
						"Integration must be a Clockify integration",
			...
			if (name !== undefined) {
				...
				if (!trimmedName)
					throw new HttpError(400, "Field `name` cannot be empty")
				values.name = trimmedName
			}
			const cadence = readOptionalBodyField(
				body,
				"cadence",
				expectInteger,
			)
			...
			const timezone = readOptionalBodyField(
				body,
				"timezone",
				expectString,
			)
			if (timezone !== undefined) {
				...
				if (!trimmedTimezone)
					throw new HttpError(400, "Field `timezone` cannot be empty")
			...
			if (status !== undefined) {
				if (
					...
					throw new HttpError(
						400,
						"Field `status` must be a valid schedule status",
					)
				}
			...
			const projectIds = readOptionalBodyField(
				body,
				"project_ids",
				expectStringArray,
			)
			...
			const finalCadence =
				(values.cadence as number | undefined) ?? schedule.cadence
			const finalTimezone =
...
export const importScheduleRunRoute = (db: Database) =>
	withErrorHandling(async (req: BunRequest<string>) => {
		...
		const job = await createJobFromSchedule(
			db,
