...
const sameZonedParts = (left: ZonedDateTimeParts, right: ZonedDateTimeParts) =>
...
const instantForZonedParts = (
	...
	const exactCandidate =
		exactCandidates.find(
			(candidate) => candidate.offset === preferredOffset,
		) ??
...
export const nextScheduleRunAt = (
	...
	if (Number.isNaN(fromDate.getTime())) {
		throw new Error(
			"Cannot calculate the next run from an invalid timestamp",
		)
	}
	if (!isValidTimezone(timezone)) {
		throw new Error(
			`Cannot calculate the next run for timezone ${timezone}`,
		)
	}
