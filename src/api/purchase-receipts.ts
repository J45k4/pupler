...
const parseCreateValues = (body: JsonObject) => {
	...
	return {
		group_id:
			readOptionalBodyField(body, "group_id", expectNullableInteger) ??
			null,
...
export const receiptsCollectionRoute = async (req: Request) => {
	...
	if (req.method === "POST") {
		...
		return json(201, await fetchReceiptDetail(db, created.id))
	}
...
export const receiptDetailRoute = async (req: BunRequest<string>) => {
	...
	if (req.method === "PUT") {
		...
		return json(200, await fetchReceiptDetail(db, id))
	}
	if (req.method === "PATCH") {
